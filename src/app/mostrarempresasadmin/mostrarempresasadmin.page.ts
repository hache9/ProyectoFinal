...
} from './../filterempresasadmin/filterempresasadmin.component';
import {
  Component,
  OnInit
} from '@angular/core';
...
export class MostrarempresasadminPage implements OnInit {
  ...
  mostrarEmpresa(empresa: UserEmpresa) {
    //console.log(empresa);
    this.router.navigate(['mostrarempresa', {
      userEmpresa: JSON.stringify(empresa)
    }]);
  }
    ...
    const edit = await this.alertController.create({
      ...
      buttons: [{
        ...
        },
        {
          text: 'Aceptar',
          handler: () => {
            this.router.navigate(['editarempresaadmin', {
