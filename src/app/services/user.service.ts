...
export class UserService {
  ...
  async userAdmin(userInfo: User) {
    ...
    this.db.doc("admin/" + userInfo.id).set({
      userInfo
    });
  }
  ...
  async userEmpresa(userInfo: UserEmpresa) {
    ...
    this.db.doc("empresa/" + userInfo.id).set({
      userInfo
    });
  }
  ...
  async userAlumno(userInfo: UserAlumno) {
    userInfo.id = this.db.createId();
    this.db.doc("alumno/" + userInfo.id).set({
      userInfo
    });
  }
  ...
  async editUserAlumno(userInfo: UserAlumno) {
      ...
      res => {
        res.forEach(element => {
          ...
          if (element.userInfo.id == userInfo.id) {
            ...
            this.db.doc("alumno/" + element.userInfo.id).update({
              userInfo
            });
          }
        });
    ...
    empresaFav.valueChanges().pipe(take(1)).subscribe(
      res => {
        res.forEach(element => {
          //console.log(element);
          this.db.doc("empresa/" + element.userInfo.id + "/Favoritos/" + userInfo.id).update({
            userInfo
          });
        });
      }
      ...
      res => {
        res.forEach(element => {
          ...
          if (element.userInfo.id == userInfo.id) {
            //console.log(userInfo.id);
            //la coleccion se crea dentro del usuario mediante la id guardada en sessionstorage
            this.db.doc("empresa/" + element.userInfo.id).update({
              userInfo
            });
          }
        });
      ...
      res => {
        res.forEach(element => {
          if (element.userInfo.id == userId) {
            this.db.doc("empresa/" + element.userInfo.id + "/Favoritos/" + userInfo.id).set({
              userInfo
            });
